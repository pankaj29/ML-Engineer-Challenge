...
Tiny-ImageNet is 64x64 with 200.
...
64x64 thumbnails, which is not what you would put behind a general-purpose
endpoint.
...
Tiny-ImageNet images are 64x64, so any larger input is an upsample. I measured
three configurations:

| Input | Stem | Top-1 | Time/epoch |
| --- | --- | ---: | ---: |
| 64x64 | adapted for small images | 73.98% | 82 s |
| 128x128 | original ImageNet | 77.66% | 38 s |
| **224x224** | original ImageNet | **78.91%** | 95 s |

The native resolution is the worst of the three. ResNet-50's stem downsamples
4x, so at 64px the stem has to be replaced, which throws away pretrained
weights and makes every later layer run at four times the spatial area. It is
less accurate and slower per epoch.

Going from 128 to 224 buys 1.25 points for 2.5x the compute. That is a small
return for a resolution change, and the reason is that upsampling adds no
information: the source is still 64x64. The ceiling is the dataset, not the
...
Dynamic quantisation produces a model that will not open, and the reason is
narrower than the error suggests. `DynamicQuantizeLinear` emits uint8 by spec;
`quantize_dynamic` defaults to int8 weights; ONNX Runtime registers its
`ConvInteger` kernel for uint8 × uint8. The mismatch fails kernel resolution
at session creation with `NOT_IMPLEMENTED`. Passing `weight_type=QUInt8`
avoids it, and the model then runs at roughly 1.44x the fp32 latency.

Static QDQ is preferred anyway, and not because of that error. Dynamic derives
activation scales from each call's own tensor, so results depend on batching.
Static calibrates once on real images, runs at 1.07x fp32 for this model, and
gives the same answer every time. The pipeline uses dynamic only when no
calibration data is available, and records that it did.
...
2. Symmetric quantisation only. Every zero point must be zero, and MinMax
...
measured against the latter, and changing what that filename contains
...
rate-limit tokens instead of 1, so it cannot be used to slip past a tier
limit.
...
artefacts, and real PostgreSQL and Redis when they are reachable. End-to-end
...
container serving an artefact the host replaced looks healthy to every
...
CI gates at 92, low enough that a failed Git LFS fetch skipping the artefact
...
### 3.7 Model artefacts use Git LFS; the dataset does not

These look like one decision and are two.

Artefacts go in, 457 MB across 9 files. They cannot be reproduced without a GPU
...
| GitHub Release assets | No history cost and 2 GB per file, but the artefacts stop being versioned alongside the code that expects them. |
| **Git LFS** (chosen) | Repository stays ~2 MB, artefacts are versioned with the code, and CI fetches them with `lfs: true`. |
...
Cache keys carry a content hash of the model artefact alongside its name and
...
results instead of crashing, so they are spelled out here.
